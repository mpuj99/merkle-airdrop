...
contract MerkleAirdrop is EIP712 {
    ...
    function claim(address account, uint256 amount, bytes32[] calldata merkleProof, uint8 v, bytes32 r, bytes32 s) external {
        ...
        // check signature
        if (!_isValidSignature(account, getMessageHash(account, amount), v, r, s)) revert MerkleAirdrop__InvalidSignature();
        
        
        
        // calculate the hash using the account and the amount --> leaf node
